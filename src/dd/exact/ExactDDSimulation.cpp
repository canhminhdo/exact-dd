...
ExactDDSimulation::ExactDDSimulation(std::size_t nqubits) : pkg_(nqubits) { reset(); }
...
ExactDDSimulation::MeasurementResult ExactDDSimulation::measure(std::size_t qubit, bool outcome) {
    const auto result = pkg_.measureOneQubit(state_, qubit, outcome);
    pkg_.incRef(result.state);
    pkg_.decRef(state_);
    state_ = result.state;
    pkg_.garbageCollect();
    return {state_, result.probability};
}
